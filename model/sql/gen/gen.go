...
import (
	...
	modelutil "github.com/yanxin666/goctlpri/model/sql/util"
...
func (g *defaultGenerator) StartFromDDL(filename string, withCache, strict bool, database, typ, name string) error {
	modelList, err := g.genFromDDL(filename, withCache, strict, database, typ)
	if err != nil {
		return err
	}

	return g.createFile(modelList, name)
}
...
func (g *defaultGenerator) createFile(modelList map[string]*codeTuple, name string) error {
...
func (g *defaultGenerator) genFromDDL(filename string, withCache, strict bool, database, typ string) (
	map[string]*codeTuple, error,
) {
	m := make(map[string]*codeTuple)
	tables, err := parser.Parse(filename, database, strict, typ)
	if err != nil {
		return nil, err
	}
